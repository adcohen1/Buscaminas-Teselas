...
def recreate_buttons():
    global buttons, btn_nuevo, btn_continuar, btn_config, btn_puntuaciones, btn_salir
    ...
    btn_nuevo = btn.Boton(w, h, cx - w//2, cy_start, "Nuevo Juego", color_base=(30, 144, 255), color_hover=(65, 105, 225))
    btn_continuar = btn.Boton(w, h, cx - w//2, cy_start + (h + gap), "Continuar", color_base=color_continuar_base, color_hover=color_continuar_hover, color_texto=color_continuar_text)
    btn_config = btn.Boton(w, h, cx - w//2, cy_start + 2*(h + gap), "Configuración")
    btn_puntuaciones = btn.Boton(w, h, cx - w//2, cy_start + 3*(h + gap), "Puntuaciones")
    btn_salir = btn.Boton(w, h, cx - w//2, cy_start + 4*(h + gap), "Salir", color_base=(185, 28, 28), color_hover=(220, 38, 38))
    
    buttons = [btn_nuevo, btn_continuar, btn_config, btn_puntuaciones, btn_salir]
    ...
    while running:
        ...
        for evt in pg.event.get():
            if evt.type == pg.QUIT:
                ...
                recreate_buttons()
                cx = conf.ancho_pantalla // 2
                btn_volver = btn.Boton(200, 50, cx - 100, int(conf.alto_pantalla * 0.7), "Volver al Menú")
                
                if overlay_exit_open:
                    cy = conf.alto_pantalla // 2
                    btn_si = btn.Boton(120, 45, cx - 140, cy + 20, "Sí", color_base=(185, 28, 28), color_hover=(220, 38, 38))
                    btn_no = btn.Boton(120, 45, cx + 20, cy + 20, "No", color_base=(71, 85, 105), color_hover=(100, 116, 139))
                    ...
                    if overlay_exit_open:
                        ...
                        if btn_nuevo.rect.collidepoint(evt.pos):
                            ...
                            if res == "QUIT":
                                ...
                                pg.display.set_caption("Teselaminas - Menú Principal")
                                recreate_buttons()
                        elif btn_continuar.rect.collidepoint(evt.pos) and game.game_active:
        ...
        if state == 0:
            ...
            rect_titulo = texto_titulo.get_rect(center=(conf.ancho_pantalla // 2, int(conf.alto_pantalla * 0.18)))
            
            # Sombra del título
            texto_sombra = fuente_titulo.render("Teselaminas", True, (2, 6, 23))
            rect_sombra = texto_sombra.get_rect(center=(conf.ancho_pantalla // 2 + 4, int(conf.alto_pantalla * 0.18) + 4))
            canva.blit(texto_sombra, rect_sombra)
            canva.blit(texto_titulo, rect_titulo)
            
            # Subtítulo descriptivo
            fuente_sub = pg.font.SysFont("Arial", 20)
            texto_sub = fuente_sub.render("El Buscaminas de Próxima Generación", True, (148, 163, 184))
            rect_sub = texto_sub.get_rect(center=(conf.ancho_pantalla // 2, int(conf.alto_pantalla * 0.26)))
            ...
            rect_wip_tit = texto_wip_tit.get_rect(center=(conf.ancho_pantalla // 2, int(conf.alto_pantalla * 0.25)))
            
            texto_wip_tit_som = fuente_wip_tit.render(wip_title_str, True, (2, 6, 23))
            rect_wip_tit_som = texto_wip_tit_som.get_rect(center=(conf.ancho_pantalla // 2 + 3, int(conf.alto_pantalla * 0.25) + 3))
            ...
            texto_wip_body = fuente_wip_body.render("TRABAJO EN PROGRESO (WIP)", True, (245, 158, 11))
            rect_wip_body = texto_wip_body.get_rect(center=(conf.ancho_pantalla // 2, int(conf.alto_pantalla * 0.4)))
            canva.blit(texto_wip_body, rect_wip_body)
            
            texto_wip_sub = fuente_wip_body.render("Esta sección estará disponible próximamente.", True, (148, 163, 184))
            rect_wip_sub = texto_wip_sub.get_rect(center=(conf.ancho_pantalla // 2, int(conf.alto_pantalla * 0.48)))
            canva.blit(texto_wip_sub, rect_wip_sub)
            
            if btn_volver:
                btn_volver.dibujar(canva)
        ...
        if overlay_exit_open:
            ...
            render_msg = fuente_box.render("¿Seguro que deseas salir?", True, (241, 245, 249))
            rect_msg = render_msg.get_rect(center=(conf.ancho_pantalla // 2, box_y + 65))
            overlay.blit(render_msg, rect_msg)
            
            if btn_si and btn_no:
                btn_si.dibujar(overlay)
                btn_no.dibujar(overlay)
